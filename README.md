...
Service 層の単体テストには Mockito でモックを活用し、外部依存を排除した純粋なロジック検証を行っています。統合テストでは H2 インメモリ DB（MySQL 互換モード）を使用することで、ローカル環境・CI 環境を問わずDBに依存しないテスト実行を実現しました。カバレッジは JaCoCo で計測し、80% 以上を目標としています。
